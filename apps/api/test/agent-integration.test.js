...
import { upsertOAuthGrant, revokeOAuthGrant } from "../src/oauth/grants.js";
...
test("GitHub agent tool is denied when required OAuth scope is missing", async () => {
  await upsertOAuthGrant({
    userId: userA.id,
    provider: "github",
    providerUserId: "github-scope-test-user",
    accessToken: "github-scope-test-token",
    scopes: ["user:email"]
  });
  ...
  assert.deepEqual(
    audit.rows[0].metadata.missingScopes,
    ["read:user"]
  );
});
test("GitHub repository lookup is denied without an active OAuth grant", async () => {
  ...
  const audit = await pool.query(
    `
      ...
      WHERE workspace_id = $1
        AND user_id = $2
        AND tool_name = 'github_get_repository'
      ORDER BY created_at DESC
      LIMIT 1
    `,
    [workspaceA.id, userA.id]
